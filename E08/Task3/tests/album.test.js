...
describe('DELETE /albums/:id', () => {
  it('should delete an album successfully', async () => {
    ...
    const albumsBefore = await Albums.find()
    const albumToDelete = albumsBefore[0]

    const res = await agent.delete(`/albums/${albumToDelete._id}`)

    expect(res.status).toBe(200)
    expect(res.body.message).toBe('Album deleted')

    const albumsAfter = await Albums.find()
    expect(albumsAfter.length).toBe(albumsBefore.length - 1)

    const deletedAlbum = await Albums.findById(albumToDelete._id)
    expect(deletedAlbum).toBeNull()
  })

  it('should handle deleting a non-existent album gracefully', async () => {
    const agent = request.agent(app)
    const loginRes = await agent.post('/auth/login').send({
      email: 'amanda@email.com',
      password: '1234567',
    ...
    const res = await agent.delete(`/albums/${fakeId}`)
